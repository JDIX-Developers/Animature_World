...
public class Database extends SQLiteOpenHelper {
	...
	public void onCreate(final SQLiteDatabase db)
	{
		...
		db
		.execSQL("CREATE TABLE Animature (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, height INTEGER, weight INTEGER, type INTEGER, type2INTEGER, speed INTEGER, defense INTEGER, agility INTEGER, strenght INTEGER, precission INTEGER, health INTEGER, level_evo INTEGER)");
		db
		...
		db
		.execSQL("CREATE TABLE Captured (id INTEGER PRIMARY KEY AUTOINCREMENT, animature INTEGER, save INTEGER, nickname TEXT, sex INTEGER, status INTEGER, capturedTime INTEGER, attack1 INTEGER, attack1_pp INTEGER, attack2 INTEGER, attack2_pp INTEGER, attack3 INTEGER, attack3_pp INTEGER, attack4 INTEGER, attack4_pp INTEGER, health INTEGER, level INTEGER, cur_exp INTEGER, exp INTEGER, box INTEGER, FOREIGN KEY(animature) REFERENCES Animature(id), FOREIGN KEY(save) REFERENCES Save(id), FOREIGN KEY(attack1) REFERENCES Attacks(id), FOREIGN KEY(attack2) REFERENCES Attacks(id), FOREIGN KEY(attack3) REFERENCES Attacks(id), FOREIGN KEY(attack4) REFERENCES Attacks(id))");
		db
		.execSQL("CREATE TABLE Bag (save INTEGER PRIMARY KEY, object INTEGER, quantity INTEGER, FOREIGN KEY(save) REFERENCES Save(id), FOREIGN KEY(object) REFERENCES Objects(id))");
		db
		.execSQL("CREATE TABLE Objects (id INTEGER PRIMARY KEY AUTOINCREMENT, type INTEGER)");
		db
		.execSQL("CREATE TABLE Enemies (id INTEGER PRIMARY KEY AUTOINCREMENT, map INTEGER, coord_x INTEGER, coord_y INTEGER, an1 INTEGER, an2 INTEGER, an3 INTEGER, an4 INTEGER, an5 INTEGER, an6 INTEGER, FOREIGN KEY(an1) REFERENCES Captured(id), FOREIGN KEY(an2) REFERENCES Captured(id), FOREIGN KEY(an3) REFERENCES Captured(id), FOREIGN KEY(an4) REFERENCES Captured(id), FOREIGN KEY(an5) REFERENCES Captured(id), FOREIGN KEY(an6) REFERENCES Captured(id))");
		db
