...
public class Database extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db)
	{
		...
		// db.execSQL("CREATE TABLE Save (id INTEGER PRIMARY KEY AUTOINCREMENT, user INTEGER, skin INTEGER, stage INTEGER, last_played DATE, started DATE, total_time INTEGER, steps INTEGER, an1 INTEGER, an2 INTEGER, an3 INTEGER, an4 INTEGER, an5 INTEGER, an6 INTEGER, map INTEGER, coord_x INTEGER, coord_y INTEGER, neighbour INTEGER, 1st_an INTEGER, orientation INTEGER, last_Healing INTEGER, medals INTEGER, money INTEGER, FOREIGN KEY(user) REFERENCES User(id), FOREIGN KEY(an1) REFERENCES Captured(id), FOREIGN KEY(an2) REFERENCES Captured(id), FOREIGN KEY(an3) REFERENCES Captured(id), FOREIGN KEY(an4) REFERENCES Captured(id), FOREIGN KEY(an5) REFERENCES Captured(id), FOREIGN KEY(an6) REFERENCES Captured(id), FOREIGN KEY(1st_an) REFERENCES Animature(id))");
